...
def build():
    ...
    for e in edges: adjacency.setdefault(e["from"],[]).append({"edge":e["id"],"to":e["to"],"relation":e["relation"]})
    for items in adjacency.values(): items.sort(key=lambda x:(x["relation"],x["to"],x["edge"]))
    return {"nodes.json":{"by_id":by_id,"by_url":by_url},"projects.json":{"projects":projects},"research.json":{"research":research},"tags.json":{"tags":dict(sorted(tags.items()))},"adjacency.json":{"adjacency":dict(sorted(adjacency.items()))},"roots.json":{"roots":[{"id":"velantrim","title":"Velantrim","type":"ECOSYSTEM"},{"id":"atlas","title":"Velantrim Knowledge Atlas","type":"DOMAIN"},{"id":"knowledge-tree-github","title":"Velantrim Knowledge Tree","type":"REPOSITORY"},{"id":"graphiti-retrieval-relevance","title":"Graphiti Fractal — Retrieval Relevance","type":"RESEARCH_THREAD"}]}}
def main():
